...
회상 도구는 정확한 프로젝트 경로를 기준으로 작업 메모를 최대 세 개까지 찾습니다. 세션 시작 또는 프롬프트 제출 훅에 연결하면, 에이전트가 관련 결정과 진행 상황을 다시 확인하도록 안내합니다. 대화 원문을 수집하거나 메모 본문을 외부로 전송하지 않으며, 저장소에 접근할 수 없으면 그 상태를 알립니다.

설정과 종료 기준은 [인수인계 안내](skills/agent-loop-os/references/handoff.md)를 참고하세요. 훅은 회상 안내를 자동으로 전달하지만, 에이전트의 실제 메모 읽기와 저장까지 보장하지는 않습니다.
